...
function enterApp(){
  ...
  if(welcome&&welcome.style.display!=="none") startWelcomeVisuals();
  input.focus();
}
$("#logout").onclick=()=>{
...
function pushHistory(text){ if(sessions[sessionId]) return; const title=text.length>42?text.slice(0,42)+"...":text; sessions[sessionId]={title,html:streamInner.innerHTML}; const item=document.createElement("button"); item.type="button"; item.className="hist-item on"; item.textContent=title; item.title=text; item.dataset.session=sessionId; item.setAttribute("role","listitem"); histEl.prepend(item); setActiveHistory(sessionId); }
...
function readOptional(obj,keys){ for(const key of keys){ const value=obj&&obj[key]; if(value!==undefined&&value!==null&&String(value).trim()!=="") return String(value); } return ""; }
function safeImageUrl(value){ return typeof value==="string"&&/^https?:\/\//i.test(value.trim())?value.trim():""; }
function metricMarkup(label,value){ return value?'<span class="rec-meta-item"><strong>'+esc(label)+':</strong> '+esc(value)+'</span>':''; }
function renderRecs(container,recs,status){ const wrap=document.createElement("div"); wrap.className="recs"; const groupId="rec-"+Math.random().toString(36).slice(2); recommendationGroups.set(groupId,recs); recs.forEach((rec,index)=>{ const pos=(rec.summary&&rec.summary.positives||[]).slice(0,3),neg=(rec.summary&&rec.summary.negatives||[]).slice(0,3); const title=rec.title||rec.product_id||"Product"; const image=safeImageUrl(readOptional(rec,["image","image_url","product_image","thumbnail","thumbnail_url"])); const price=readOptional(rec,["price","price_text"]),rating=readOptional(rec,["rating","rating_score"]),match=readOptional(rec,["match_score","matchScore","match_percentage","match"]),review=readOptional(rec,["review_summary","reviewSummary","review"]); const badge=index===0?"Best Match":index===1?"Smart Save":index===2?"Upgrade":""; const card=document.createElement("button"); card.type="button"; card.className="rec"; card.dataset.recGroup=groupId; card.dataset.recIndex=String(index); card.setAttribute("aria-label","Browse recommendation "+(index+1)+": "+title); card.setAttribute("aria-pressed","false"); card.innerHTML='<div class="rec-media">'+(image?'<img src="'+esc(image)+'" alt="" loading="lazy">':'<span class="rec-media-fallback">Image not provided</span>')+(badge?'<span class="rec-badge">'+badge+'</span>':'')+'</div><div class="rec-body"><div class="rec-head"><div><div class="rec-title">'+esc(title)+'</div><div class="rec-open-hint">Open shopping browser</div></div><div class="rec-rank">#'+String(index+1).padStart(2,"0")+'</div></div><div class="rec-reason">'+esc(rec.reason||"Recommendation reason not provided.")+'</div><div class="rec-meta">'+metricMarkup("Price",price)+metricMarkup("Rating",rating)+metricMarkup("Match",match)+'</div><div class="rec-summary"><div class="sum"><div class="sum-h">Positives</div><ul>'+(pos.map(v=>"<li>"+esc(v)+"</li>").join("")||"<li>Not provided</li>")+'</ul></div><div class="sum"><div class="sum-h">Negatives</div><ul>'+(neg.map(v=>"<li>"+esc(v)+"</li>").join("")||"<li>Not provided</li>")+'</ul></div></div>'+(review?'<div class="rec-review"><strong>Review summary</strong>'+esc(review)+'</div>':'')+'<span class="rec-link">View Product</span></div>'; wrap.appendChild(card); }); container.appendChild(wrap); if(status==="insufficient")renderNotices(container,["Few items matched. Showing all available results."]); scrollDown(); }
...
function addThinking(){ const message=document.createElement("div");message.className="msg ai";message.innerHTML='<div class="avatar ai">N</div><div class="bubble"><div class="thinking" role="status" aria-live="polite"><span class="thinking-mark">N</span><span class="thinking-copy"><span class="thinking-label">Nova is reviewing your request…</span><span class="thinking-stages"><span class="thinking-stage current">搜索商品</span><span class="thinking-stage">比较价格</span><span class="thinking-stage">分析评论</span><span class="thinking-stage">生成推荐</span></span></span></div></div>';streamInner.appendChild(message);scrollDown();return message; }
...
streamInner.addEventListener("click",event=>{const chip=event.target.closest(".chip"),option=event.target.closest(".opt"),card=event.target.closest(".rec"),history=event.target.closest(".hist-item"),profileEdit=event.target.closest(".profile-edit");if(profileEdit&&!busy){input.value="I’d like to update my shopping profile";autosize();syncSendState();input.focus();return;}if(chip&&!busy){input.value=chip.textContent;autosize();doSend();}if(option&&!busy&&!option.parentElement.classList.contains("done")){const group=option.parentElement;[...group.children].forEach(button=>{button.classList.toggle("picked",button===option);button.setAttribute("aria-pressed",button===option?"true":"false");});group.classList.add("done");input.value=option.textContent;doSend();}if(card){const recs=recommendationGroups.get(card.dataset.recGroup);const index=Number(card.dataset.recIndex);if(recs&&Number.isInteger(index)){[...card.parentElement.querySelectorAll(".rec")].forEach(button=>{button.classList.toggle("active",button===card);button.setAttribute("aria-pressed",button===card?"true":"false");});activeRecCard=card;productPanelOpener=card;openRecommendationsPanel(recs);selectProduct(index);setTimeout(()=>$("#ppClose").focus(),0);}}if(history)switchToSession(history.dataset.session);});
...
/* ---- lifecycle wiring ---- */
let hero=null,welcomeOrb=null,welcomeSpot=null;
...
/* in-app chat welcome visuals (authenticated product) - unchanged */
function startWelcomeVisuals(){
  const fx=$("#welcomeFx"); if(!fx) return;
  if(!welcomeOrb){ const c=$("#welcomeOrb"); if(c) welcomeOrb=makeOrb(c,{mode:"sparse",count:220,links:true}); }
  if(!welcomeSpot){ welcomeSpot=makeSpotlight(fx,fx.querySelector(".welcome-spot")); }
}
function stopWelcomeVisuals(){
  if(welcomeOrb){ welcomeOrb.destroy(); welcomeOrb=null; }
  if(welcomeSpot){ welcomeSpot.destroy(); welcomeSpot=null; }
}
